...
class BOMJoinedRow:
    ...
    category_name: str = ""
    # BD_MATERIAL.MaterialBase.IsPurchase boolean (via FMaterialId.FIsPurchase).
    # Splits the coarse 外销包材 category: True = truly purchased packaging
    # (外箱/内盒/纸卡), False = self-made plastic packaging parts (吸塑/跟型件).
...
class CacheReader:
    ...
    async def get_production_bom(self, bill_nos: list[str]) -> CacheResult:
        ...
        rows = await self.db.execute_read(
            ...
            SELECT mo_bill_no, mto_number, material_code, material_name,
                   ...
                   category_name, is_purchase, synced_at
        ...
        # synced_at is now at index 14 (material_group_name=11, category_name=12, is_purchase=13)
        synced_times = [self._parse_timestamp(row[14]) for row in rows if row[14]]
    ...
    async def get_mto_bom_joined(self, mto_number: str) -> CacheResult:
        ...
        # synced_at is the last column (index 30 — after 6 telemetry CASE columns
        # at 21-26, material_group_name at 27, category_name at 28, is_purchase at 29)
        synced_times = [self._parse_timestamp(row[30]) for row in rows if row[30]]
        oldest_sync = min(synced_times) if synced_times else None
        is_fresh = self._is_fresh(oldest_sync) if oldest_sync else False
    ...
    def _row_to_bom(self, row: tuple) -> ProductionBOMModel:
        ...
        4: specification, 5: aux_attributes, 6: aux_prop_id, 7: material_type,
        8: need_qty, 9: picked_qty, 10: no_picked_qty,
        11: material_group_name, 12: category_name, 13: is_purchase, 14: synced_at
        """
